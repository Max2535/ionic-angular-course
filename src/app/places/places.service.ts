...
export class PlacesService {
   ...
   return this.authService.token.pipe(
    ...
    switchMap(token => {
      return this.http.get<{ [key: string]: PlaceData }>(
        `https://ionic-angular-course-d7320.firebaseio.com/offered-places.json?auth=${token}`
      )
    }), map(resData => {
      const places = [];
  ...
  getPlace(id: string) {
    return this.authService.token.pipe(
      take(1),
      switchMap(token => {
      return this.http.get<PlaceData>(`https://ionic-angular-course-d7320.firebaseio.com/offered-places/${id}.json?auth=${token}`)
    }),
        map(placeData => {
          return new Place(
            id,
    ...
    return this.authService.token.pipe(
      ...
      switchMap(places => {
        ...
        return this.http.put(
          `https://ionic-angular-course-d7320.firebaseio.com/offered-places/${placeId}.json?auth=${fetchedToken}`,
